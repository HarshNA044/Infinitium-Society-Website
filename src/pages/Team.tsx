...
import { parseTenureToSingleYears, formatTenureDisplayTags } from "../lib/utils";
...
export default function Team_Page() {
    ...
    const fetchTeam = async () => {
      try {
        ...
        if (data.length > 0) {
          ...
          data.forEach((m: any) => {
            parseTenureToSingleYears(m.tenure).forEach((yr) => yearsSet.add(yr));
          });
          const sortedYears = Array.from(yearsSet).sort().reverse();
          if (sortedYears.length > 0) {
            // Default to checking the latest active academic year
            setSelectedTenures([sortedYears[0]]);
          }
        }
      } catch (error) {
  ...
  // Compile all unique single-year tenures across all team members
  const availableYears = Array.from(
    new Set(
      allMembers.flatMap((m: any) => parseTenureToSingleYears(m.tenure))
    )
  ).sort().reverse();

  // Filter members: check if they are active in ANY of the checked years
  const filteredMembers = allMembers.filter((m: any) => {
    const memberYears = parseTenureToSingleYears(m.tenure);
    return memberYears.some((yr) => selectedTenures.includes(yr));
  });

  // Custom role priority mapping
  const getRolePriority = (role: string): number => {
    ...
    if (r.includes("sub-head") || r.includes("subhead") || r.includes("sub head")) return 6;
    if (r.includes("head")) return 5;
    return 7; // general members / other roles
  };

  // Sort: Role Priority (ascending), then alphabetically by name
  const sortedFilteredMembers = [...filteredMembers].sort((a: any, b: any) => {
      ...
      <div className="max-w-7xl mx-auto">
        <header className="mb-12 text-center max-w-2xl mx-auto flex flex-col items-center">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-slate-900 tracking-tighter leading-none mb-4 uppercase text-center">
            The Minds Behind <br />{" "}
            <span className="text-brand-600">INFINITIUM</span>
          </h1>
        </header>
          ...
          <aside className="lg:w-72 flex-shrink-0">
            <div className="sticky top-32 space-y-6">
              <div className="p-8 bg-zinc-50 rounded-2xl border border-zinc-100 shadow-sm transition-all hover:shadow-xl hover:shadow-brand-600/5">
                <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-6 pl-1 border-b border-zinc-100 pb-2">
                  Filter Year Wise
                </h3>
                <div className="space-y-3.5">
                  {availableYears.map((yr) => {
                    const isChecked = selectedTenures.includes(yr);
                    return (
                      <label
                        key={yr}
                        className="flex items-center gap-3 cursor-pointer select-none text-xs font-black text-slate-700 hover:text-black transition-colors"
                      >
                        <input
                          type="checkbox"
                          checked={isChecked}
                          onChange={(e) => {
                            if (e.target.checked) {
                              setSelectedTenures((prev) => [...prev, yr]);
                            } else {
                              setSelectedTenures((prev) =>
                                prev.filter((item) => item !== yr),
                              );
                            }
                          }}
                          className="w-5 h-5 rounded text-brand-600 focus:ring-brand-500 border-zinc-300 cursor-pointer"
                        />
                        <span>{yr}</span>
                      </label>
                    );
          ...
          <div className="flex-1">
              ...
              <h2 className="text-xl font-black text-slate-900 uppercase tracking-tighter">
                {selectedTenures.length === 0
                  ...
                  : selectedTenures.length === 1
                    ? `${selectedTenures[0]} Active Team`
                    : `Active Team (${selectedTenures.join(", ")})`}
              </h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {sortedFilteredMembers.map((member: any, idx: number) => (
                ...
                >
                  {/* Decorative gradient semi-circle corner patterns */}
                  {/* Top-left corners */}
                  ...
                  <div className="relative overflow-hidden rounded-lg bg-zinc-100 aspect-square mb-2.5 z-10">
                    <img
                      src={member.image || "https://i.ibb.co/cKD8BFhN/logo.png"}
                      alt={member.name}
                  ...
                  <div className="flex flex-col flex-grow text-center z-10">
                    ...
                    <div className="mt-1 mb-1.5 flex flex-col items-center gap-1">
                      ...
                      <span className="text-[7.5px] text-slate-400 font-bold uppercase tracking-wider">
                        {formatTenureDisplayTags(member.tenure)}
                      </span>
                    </div>

                    <div className="mt-auto space-y-0.5 border-t border-zinc-100/50 pt-2">
                      {member.year ? (
            ...
            {sortedFilteredMembers.length === 0 && (
              <div className="py-20 bg-zinc-50 rounded-2xl border-2 border-dashed border-zinc-100 text-center flex flex-col items-center">
                ...
                <p className="text-slate-400 font-black uppercase text-xs tracking-widest">
                  No active team members checked.
                </p>
              </div>
            )}
          </div>
